...
void Framebuffer::draw_line(int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t color) {
    // Basic clipping: If both points are far off screen, skip
    if ((x1 < -1000 || x1 > 3000) || (y1 < -1000 || y1 > 3000)) return;
    if ((x2 < -1000 || x2 > 3000) || (y2 < -1000 || y2 > 3000)) return;
    ...
    while (true) {
        // Use inlined put_pixel for safety - the compiler will optimize this
