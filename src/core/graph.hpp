...
public:
    ...
    void draw_2d(Framebuffer& fb, std::function<float(float)> func, float x_min, float x_max, float y_min, float y_max, const Color& c) {
        float px = -1, py = -1;
    ...
    void draw_axes(Framebuffer& fb, float x_min, float x_max, float y_min, float y_max, const Color& c) {
        if (y_min <= 0 && y_max >= 0) {
